...
/*! \enum CF_IMAGE_DRAW_OPTIONS 
	\brief тип отрисовки изображения  

*/
enum CF_IMAGE_DRAW_OPTIONS {
	doBestFit = 0,						/** вписать изображение в прямоугольник окна сохраняя отношения сторон */
	doStretch,    					    /** вписать изображение в прямоугольник окна не сохраняя отношения сторон*/
};
typedef CF_HANDLE CF_IMAGE;            /*дескриптор изображения*/    
typedef CF_HANDLE CF_MEDIA_SOURCE;	   /*деккриптор источника изображений. Источниками изображений могут быть
										- устройства ввода, видеофайлы*/
extern "C"
{
...
/*!\fn CF_ERROR __EXPORT_TYPE CFDrawImage(CF_IMAGE hImage)
   \brief отрисовывает изображение hImage на устройстве CanvasHandler
	
	отрисовывает изображение hImage на устройстве CanvasHandler с 
	с учетом опций DrawOptions 

	\param hImage		 - дескриптор изображения.
    \param CanvasHandler - идентификатор устройства. В среде Windows это hWnd 
