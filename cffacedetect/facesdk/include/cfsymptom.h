...
*/
enum CF_SYMPTOM_BLOB_DETECTOR {
	sbdSG = 0,					/** использует разницу моделью фона на основе гаусса*/
	sbdPCA					    /** использует разницу между моделью фона на основе PCA*/
};

/*! \enum CF_SYMPTOM_PROCESSED_IMAGE_TYPE 
	\brief тип предобработанного изображеия 

*/
enum CF_SYMPTOM_PROCESSED_IMAGE_TYPE {
	pitBackground = 0,					/** модель фона */
	pitDiff,    					    /** вероятностная картина переднего плана*/
	pitBinaryForSearch,					/** изображение, на котором ищутся новые объекты*/
	pitBinaryForTrack, 					/** изображение, на котором ведется сопровождение объектов*/
	pitRenderOutput,						/** изображение, на котором отрисованы ROI, все объекты, их ц.м. и 
										области поиска*/
	pitRenderMesurer					/*изображение, на котором отрисована измерительная линейка*/
};

struct CF_SYMPTOM_CAMERA
{
	/*!\brief size - this stucture identifier. 
	  should be always = sizeof(CF_SYMPTOM_CAMERA)
	*/
	int size;
	float WChipSize;
	float HChipSize;
...
struct CF_SYMPTOM_PARAMS
{
	...
	*/
	...
	/*!\brief  number of training frames
	
	 the number of frames required to train the model background
	 default value - 100*/
	int  number_frames_for_trinig;
	/*!\brief  порог обнаружения объектов переднего плана
	
	порог для обнаружения объектов переднего плана
	допустимые значения of 0 .. 1
	0 - означает что вернется изображение, полностью состоящее из
	переднего плана
	1 - означает что вернется изображение, полностью состоящее из 
	фона
	значение по умолчанию - 0.1
    */
     float fgbg_threshold;
	/*!\brief  порог изменения линейного размера по х в % от старого 
	размера
	
	порог изменения линейного размера по х. Если линейный размер 
	объекта на новом изображении превышает этот порог, то будет использован 
	старый линейный размер
    ...
    */
	...
	/*!\brief  порог изменения линейного размера по y в % от старого 
	размера
	
	порог изменения линейного размера по y. Если линейный размер 
	объекта на новом изображении превышает этот порог, то будет использован 
	старый линейный размер
    ...
    */
	...
	/*!\brief  размер области поиска в % от старого размера по Х 
	
	размер области поиска нового местоположения объекта в % 
	от старого размер объекта
    ...
    */
	 ...
	 CF_SYMPTOM_CAMERA camera_options;
};
...
struct CF_SYMPTOM_CAMERA_RENDER_OPTIONS
{
	/*!\brief size - this stucture identifier. 
	  should be always = sizeof(CF_SYMPTOM_CAMERA_RENDER_OPTIONS)
	*/
	int size;
	CF_SYMPTOM_CAMERA		camera;
	CF_CAMERA_RENDER_TYPE	render_type;
	CF_IMAGE				render_canvas;
	float					measurer_length;
	int						x;
	int                     y;
};
...
/*!\fn  CF_ERROR __EXPORT_TYPE  CFResetSymptom(CF_SYMPTOM hSymptom)
   \brief сброс системы видеонаблюдения
	
	Сбрасывает машину видеонаблюдения, устранавливает новую модель фона, 
	обнуляет число известных объектов. 

	\param hSymptom - дескриптор модуля видеонаблюдения 

	Return values: CF_SUCCESS операция выполнена успешно \n
	CFF_INVALID_HANDLE  если hSymptom  == NULL или недействительный \n
*/
CF_ERROR __EXPORT_TYPE  CFResetSymptom(CF_SYMPTOM hSymptom);
...
/*!\fn  CF_ERROR __EXPORT_TYPE  CFResetSymptom(CF_SYMPTOM hSymptom)
   \brief сброс системы видеонаблюдения
	...
	CFF_INVALID_HANDLE  если hSymptom  == NULL или недействительный \n
*/
CF_ERROR __EXPORT_TYPE  CFGetProcessedImage(CF_SYMPTOM hSymptom, CF_IMAGE hImage, CF_SYMPTOM_PROCESSED_IMAGE_TYPE ImageType);

CF_ERROR __EXPORT_TYPE CFRenderCameraOptions(CF_SYMPTOM_CAMERA_RENDER_OPTIONS* pOptions);
